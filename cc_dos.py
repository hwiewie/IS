...
def mk_threading(proxies, url ,jobs, threadnum ,method ,post):
	...
	try:
		result = threadsPool.map(cc_website , map(lambda x:(x, url, jobs, method, post), proxies)) #�ؼ�����
		threadsPool.close()
		threadsPool.join()
	except Exception, e:
		pass
...
'''
cc main function create new test process
'''
...
'''
vist baidu.com to check proxy is avalivable
'''
